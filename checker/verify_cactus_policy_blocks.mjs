// JavaScript counterpart of verify_cactus_policy_blocks.ts.
import {
  createReadStream,
  existsSync,
  readFileSync,
...
var MAX_VERTICES = 12;
var observations = [];
var observationByKey = /* @__PURE__ */ new Map();
function observationKey(current, counts) {
  ...
  for (let current = 0; current < COLORS; current += 1) {
    for (let degree = 1; degree <= 3; degree += 1) {
      for (let color0 = 0; color0 <= degree; color0 += 1) {
        for (let color1 = 0; color1 <= degree - color0; color1 += 1) {
          const counts = [
            color0,
            color1,
...
var Graph = class {
  constructor(graph6) {
    ...
    const bits = [];
    for (let index = 1; index < graph6.length && bits.length < requiredBits; index += 1) {
  ...
  colorsOf(physical) {
    ...
    const colors = Array(this.n).fill(0);
  ...
  fullState(physical, visited) {
    return physical * this.masks + visited;
  }
  nextState(state, nextPhysical) {
...
function parsePartialPolicy(clause) {
  const partial = new Int8Array(OBSERVATIONS);
  partial.fill(-1);
  for (const literal of clause) {
    if (!Number.isInteger(literal) || literal >= 0 || -literal > OBSERVATIONS * ACTIONS) {
    ...
    if (partial[observation] >= 0 && partial[observation] !== action) {
      return "TAUTOLOGY";
    }
...
function strictActionIsValid(observation, action) {
  return action === STOP || observations[observation].counts[action % COLORS] !== 0;
}
function strictActionMask(observation) {
  let result = 1 << STOP;
...
function domainForcesFailure(graph, domain, start) {
  ...
  for (let state = 0; state < fullStates; state += 1) {
    ...
    if ((actions & 1 << STOP) !== 0 && graph.positionOf(physical) === start && visited === graph.masks - 1) {
      winning[state] = 1;
      queue.push(state);
    }
    for (let action = 0; action < STOP; action += 1) {
  ...
  for (let state = 0; state < fullStates; state += 1) {
    reverseOffsets[state + 1] = reverseOffsets[state] + reverseCounts[state];
  }
  ...
  for (let head = 0; head < queue.length; head += 1) {
    ...
    for (let offset = reverseOffsets[won]; offset < reverseOffsets[won + 1]; offset += 1) {
...
function domainForcesFailureNaive(graph, domain, start) {
  const fullStates = graph.n * graph.colorings * graph.masks;
  const winning = new Uint8Array(fullStates);
  let changed = true;
  while (changed) {
    ...
    for (let state = 0; state < fullStates; state += 1) {
      ...
      const observation = graph.observationOf(physical);
      const actions = domain[observation];
      let stateWins = (actions & 1 << STOP) !== 0 && graph.positionOf(physical) === start && visited === graph.masks - 1;
      for (let action = 0; action < STOP && !stateWins; action += 1) {
        if ((actions & 1 << action) === 0) continue;
        const successors = graph.transitionOf(
          physical,
          action
        ...
        if (successors.length === 0) continue;
        stateWins = successors.every(
          (nextPhysical) => winning[graph.nextState(state, nextPhysical)] !== 0
        );
      }
...
function generalizeTraceDomain(graph, clause, start) {
  const domain = traceDomain(clause);
  if (domain === "TAUTOLOGY") return void 0;
...
function expectedDomainBlock(domain) {
  const result = [];
  for (let observation = 0; observation < OBSERVATIONS; observation += 1) {
    const complement = strictActionMask(observation) & ~domain[observation];
    for (let action = 0; action < ACTIONS; action += 1) {
...
function graphWitness(graph, partial, start) {
  const initial = graph.fullState(
    start * graph.colorings,
    1 << start
  );
  const stateIndex = /* @__PURE__ */ new Map([[initial, 0]]);
  ...
  for (let head = 0; head < states.length; head += 1) {
    ...
    if (action === STOP) {
      if (graph.positionOf(physical) !== start || visited !== graph.masks - 1) {
        return {
          graph6: graph.graph6,
          start,
          kind: "BAD_STOP",
...
function findWitness(graphs, clause) {
  const partial = parsePartialPolicy(clause);
  if (partial === "TAUTOLOGY") return partial;
  for (const graph of graphs) {
    for (let start = 0; start < graph.n; start += 1) {
      const witness = graphWitness(graph, partial, start);
      if (witness !== void 0) return witness;
    }
  }
  return void 0;
}
function loadGraphs(path, added) {
  const graph6s = readFileSync(path, "utf8").split(/\r?\n/).map((line) => line.trim()).filter((line) => line.length > 0 && !line.startsWith("#"));
  graph6s.push(...added);
  if (graph6s.length === 0) throw new Error("graph list is empty");
  return graph6s.map((graph6) => new Graph(graph6));
}
async function verifyBlocks(graphs, path, limit, mutationTest, verbose, generalizeMaxOrder, filterOutput) {
  ...
  for await (const rawLine of input) {
    ...
    if (line.startsWith("c graph-gfp-domain ")) {
      if (pendingDomain !== void 0) {
        throw new Error(
          `missing graph domain clause before line ${lineNumber}`
        );
      }
      const match = /^c graph-gfp-domain graph6=(\S+) start=(\d+) masks=(.+)$/.exec(line);
      if (match === null) {
        throw new Error(
      ...
      if (masks.length !== OBSERVATIONS || masks.some((mask, observation) => !Number.isInteger(mask) || mask <= 0 || mask >= 1 << ACTIONS || (mask & ~strictActionMask(observation)) !== 0)) {
        throw new Error(
    ...
    if (clause.length === 0 || clause.some((literal) => !Number.isInteger(literal) || literal === 0)) {
    ...
    if (pendingDomain !== void 0) {
      ...
      if (graph === void 0) {
        ...
        keep = false;
      } else if (pendingDomain.start < 0 || pendingDomain.start >= graph.n) {
        throw new Error(
      ...
      if (keep && mutationTest && !mutationRejected) {
        for (let removed = 0; removed < clause.length; removed += 1) {
          ...
          const observation = Math.floor(zeroBased / ACTIONS);
          const action = zeroBased % ACTIONS;
          const mutatedDomain = new Uint16Array(pendingDomain.domain);
      ...
      if (witness === void 0) {
        ...
        if (verbose) {
          console.log(
            `block=${verified + 1} graph6=${witness.graph6} start=${witness.start} kind=${witness.kind} reachable=${witness.reachable}`
          );
        }
        if (generalizeMaxOrder > 0) {
          ...
          if (graph.n <= generalizeMaxOrder) {
            ...
            if (result !== void 0) {
              ...
              if (verbose) {
                console.log(
                  `domain-generalization block=${verified + 1} initial-complement=${result.initialComplement} final-complement=${result.finalComplement} added-actions=${result.addedActions} oracle-calls=${result.oracleCalls}`
                );
              }
            }
          } else {
      ...
      if (keep && mutationTest && !mutationRejected && clause.length > 1) {
        for (let removed = 0; removed < clause.length; removed += 1) {
          ...
          if (findWitness(graphs, mutation) === void 0) {
            mutationRejected = true;
            break;
          }
        }
...
`);
  ...
  console.log(
    `cactus-policy-blocks=${filterOutput.length > 0 ? "FILTERED" : "VERIFIED"} blocks=${verified} skipped=${skipped} literals=${literals} graphs=${graphs.length} tautologies=${tautologies} mutation-rejected=${mutationRejected ? 1 : 0} domain-generalized=${generalized} domain-skipped=${generalizationSkipped} domain-certificates=${domainCertificates} initial-complement=${initialComplement} final-complement=${finalComplement} domain-oracle-calls=${domainOracleCalls} file=${path}`
  );
}
async function main() {
  ...
  for (let index = 2; index < process.argv.length; index += 1) {
    ...
    } else if (argument === "--add-graph" && index + 1 < process.argv.length) {
      addedGraphs.push(process.argv[++index]);
    } else if (argument === "--blocks" && index + 1 < process.argv.length) {
      blocksPaths.push(process.argv[++index]);
    } else if (argument === "--limit" && index + 1 < process.argv.length) {
      limit = Number(process.argv[++index]);
    } else if (argument === "--mutation-test") {
      mutationTest = true;
    } else if (argument === "--domain-generalize-max-order" && index + 1 < process.argv.length) {
      generalizeMaxOrder = Number(process.argv[++index]);
    } else if (argument === "--domain-self-test" && index + 1 < process.argv.length) {
      domainSelfTest = Number(process.argv[++index]);
    } else if (argument === "--verbose") {
      verbose = true;
    } else if (argument === "--filter-output" && index + 1 < process.argv.length) {
      filterOutput = process.argv[++index];
    } else {
      throw new Error(`unknown or incomplete argument: ${argument}`);
    }
  }
  if (graphsPath.length === 0 || blocksPaths.length === 0 || !Number.isInteger(limit) && limit !== Number.POSITIVE_INFINITY || limit <= 0) {
    throw new Error(
      "usage: verify_cactus_policy_blocks.ts --graphs FILE [--add-graph GRAPH6] --blocks FILE [--limit N] [--mutation-test] [--domain-generalize-max-order N] [--domain-self-test N] [--verbose] [--filter-output FILE]"
    );
  }
  if (filterOutput.length > 0) {
